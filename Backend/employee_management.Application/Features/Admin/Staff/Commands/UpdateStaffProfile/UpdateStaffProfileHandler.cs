...
{
    public sealed class UpdateStaffProfileHandler : IRequestHandler<UpdateStaffProfileRequest, UpdateStaffProfileResponse>
    {
        ...
        public UpdateStaffProfileHandler(
            IUnitOfWork unitOfWork,
            IEmployeeRepository employeeRepository,
            UserManager<User> userManager,
            ILogger<UpdateStaffProfileHandler> logger)
        {
        ...
        public async Task<UpdateStaffProfileResponse> Handle(UpdateStaffProfileRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (employee == null)
                {
                    _logger.LogWarning("Employee with Id: {EmployeeId} not found for staff profile update", request.StaffId);
                    throw new NoDataFoundException($"Employee with Id {request.StaffId} not found.");
                }

                employee.Name = request.FullName.Trim();
                ...
                else if (!string.IsNullOrWhiteSpace(request.ProfileImageDataUrl))
                {
                    employee.Avatar = request.ProfileImageDataUrl;
                }
                // else: keep existing avatar unchanged

                _employeeRepository.Update(employee);
                await _unitOfWork.Save(cancellationToken);

                // Find linked user for response
                ...
                return new UpdateStaffProfileResponse(dto);
            }
            ...
            {
                _logger.LogError(ex, "Error updating staff profile for EmployeeId: {EmployeeId}", request.StaffId);
                throw;
            }
        }
    }
